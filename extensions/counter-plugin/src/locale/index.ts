...
export function registerCounterLocale(): void {
  const i18n = getHost();
  if (!i18n) return;
...
export function counterT(key: string, params: Params = {}): string {
  registerCounterLocale();
  const i18n = getHost();
...
export const t = counterT;
export const counterLocale = resources;
export { en, zhCN };
