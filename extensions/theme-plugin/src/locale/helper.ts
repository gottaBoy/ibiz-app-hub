...
export type ThemePluginLocaleKey = keyof typeof zhCN.themePlugin;
...
export const themePluginLocale = {
  'zh-CN': zhCN,
  en,
} as const;
...
const markerPrefix = '__theme-plugin-locale:';
...
export function normalizeThemePluginLocale(language: unknown): Locale {
  const normalized =
...
function missingMessages(
  ...
  Object.entries(data).forEach(([name, value]) => {
    ...
    if (typeof value === 'string') {
      ...
      const other = normalizeThemePluginLocale(language) === 'en' ? zhCN : en;
      if (!isUsableTranslation(existing, tag) ||
        existing === other.themePlugin[name as ThemePluginLocaleKey]) result[name] = value;
      return;
    }
...
export function registerThemePluginLocale(): boolean {
  ...
  try {
    ...
    languages.forEach(language => {
      ...
      try {
        const data = themePluginLocale[normalizeThemePluginLocale(language)];
...
export function themePluginT(
  key: ThemePluginLocaleKey,
  params: Params = {},
): string {
  registerThemePluginLocale();
  const i18n = getHostI18n();
  const language = getLanguage(i18n);
  const messages = themePluginLocale[normalizeThemePluginLocale(language)];
  const fallback = interpolate(messages.themePlugin[key], params);
  const tag = `themePlugin.${String(key)}`;
  try {
...
export const t = themePluginT;

export function themePluginCaption(key: ThemePluginLocaleKey): string {
  return `${markerPrefix}${String(key)}`;
}

function markerKey(value: unknown): ThemePluginLocaleKey | undefined {
  if (typeof value !== 'string' || !value.startsWith(markerPrefix)) {
    return undefined;
  }
  const key = value.slice(markerPrefix.length) as ThemePluginLocaleKey;
  return Object.prototype.hasOwnProperty.call(zhCN.themePlugin, key)
    ? key
    : undefined;
}

/**
 * Clone a generated layout while keeping plugin-owned labels live.
 * User and protocol values are copied as-is.
 */
export function localizeThemeLayoutModel<T>(model: T): T {
  if (Array.isArray(model)) {
    return model.map(item => localizeThemeLayoutModel(item)) as T;
  }
  if (!model || typeof model !== 'object') return model;

  const result: Messages = {};
  Object.entries(model as Messages).forEach(([name, value]) => {
    const key = name === 'caption' ? markerKey(value) : undefined;
    if (key) {
      Object.defineProperty(result, name, {
        configurable: true,
        enumerable: true,
        get: () => themePluginT(key),
      });
      return;
    }
    result[name] = localizeThemeLayoutModel(value);
  });
  return result as T;
}
