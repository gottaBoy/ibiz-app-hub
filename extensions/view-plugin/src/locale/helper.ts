import { viewPluginEn } from './en';
import { viewPluginZhCN } from './zh-CN';
...
export const viewPluginLocale = {
  'zh-CN': viewPluginZhCN,
  en: viewPluginEn,
} as const;
...
export function normalizeViewPluginLocale(language: string): Locale {
...
function missingMessages(
  ...
  Object.entries(messages).forEach(([key, value]) => {
    ...
    if (typeof value === 'string') {
      ...
      const other =
        normalizeViewPluginLocale(language) === 'en' ? 'zh-CN' : 'en';
      if (
        !isHostTranslation(existing, tag, key) ||
        existing === lookup(viewPluginLocale[other], tag)
      ) {
        result[key] = value;
      }
      return;
...
export function registerViewPluginLocale(): void {
  ...
  try {
    ...
    targetLanguages.forEach(language => {
      ...
      try {
        const locale = normalizeViewPluginLocale(language);
        const messages = missingMessages(
          i18n,
          viewPluginLocale[locale],
          language,
        );
...
export function viewPluginT(key: string, params?: TranslationParams): string {
  const i18n = getHostI18n();
  const locale = normalizeViewPluginLocale(getLanguage(i18n));
  const fallback = lookup(viewPluginLocale[locale], `viewPlugin.${key}`);
  const fallbackMessage =
    typeof fallback === 'string' ? interpolate(fallback, params) : key;
  const tag = `viewPlugin.${key}`;
  ...
  const other = lookup(viewPluginLocale[otherLocale], `viewPlugin.${key}`);
...
export const t = viewPluginT;
export const registerLocale = registerViewPluginLocale;
