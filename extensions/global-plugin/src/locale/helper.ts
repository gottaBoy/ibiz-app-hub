...
type TranslationKey = keyof typeof zhCN.globalPlugin;
...
export const globalPluginLocale = {
  'zh-CN': zhCN,
  en,
} as const;
...
export function normalizeGlobalPluginLocale(language: unknown): Locale {
  const normalized =
...
function missingMessages(
  ...
  Object.entries(data).forEach(([name, value]) => {
    ...
    if (typeof value === 'string') {
      ...
      const other = normalizeGlobalPluginLocale(language) === 'en' ? zhCN : en;
      if (!isUsableTranslation(existing, tag) ||
        existing === other.globalPlugin[name as TranslationKey]) result[name] = value;
      return;
    }
...
export function registerGlobalPluginLocale(): boolean {
  ...
  try {
    ...
    languages.forEach(language => {
      ...
      try {
        const data =
          globalPluginLocale[normalizeGlobalPluginLocale(language)];
...
export function globalPluginT(
  ...
  const messages = globalPluginLocale[normalizeGlobalPluginLocale(language)];
  const fallback = interpolate(messages.globalPlugin[key], params);
  const tag = `globalPlugin.${String(key)}`;
  try {
