...
export function registerDeActionLocale(): void {
  const i18n = getHost();
  if (!i18n) return;
...
export function deActionT(key: string, params: Params = {}): string {
  registerDeActionLocale();
  const i18n = getHost();
...
export const t = deActionT;
export const deActionLocale = resources;
export { en, zhCN };
