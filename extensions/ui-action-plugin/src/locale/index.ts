...
export function registerUiActionLocale(): void {
  const i18n = getHost();
  if (!i18n) return;
...
export function uiActionT(key: string, params: Params = {}): string {
  registerUiActionLocale();
  const i18n = getHost();
...
export const t = uiActionT;
export const uiActionLocale = resources;
export { en, zhCN };
