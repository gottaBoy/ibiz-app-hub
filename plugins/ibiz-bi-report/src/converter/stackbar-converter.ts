 ...
 */
export class StackBarConverter extends BaseConverter {
  /**
   * 条形图数据转模型
   *
    ...
    if (data) {
      ...
      model.dechartSerieses?.forEach((series: any) => {
        Object.assign(series, {
          userParam: {
            ...this.computeAxisLayout(series, uiModel, true),
            ...this.computeMaxMin(seriesParams, items, series),
            ...handleSerieGroupTip(serieGroup, series, true, true),
          },
        });
      });
    }
  ...
  transformStyle(
    ...
    if (style) {
      ...
      if (label) {
        ...
        seriesParams['EC.stack'] = 'stackbar';
      }
      if (legend) {
