import { defineComponent, ref, watch, Ref, nextTick } from 'vue';
...
export const IBizSignature = defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const handleRemove = (_evt: MouseEvent): void => {
     ...
     * @return {*}
     */
    const handlePadContainerClick = (): void => {
      if (props.disabled || props.readonly) fullScreen.value = false;
    };
    ...
    const handleConfirm = (): void => {
      ...
      // 当为空白，或者未进行过签名时，不保存
      if (
        signatureRef.value?.signaturePad.isEmpty() ||
        !signatureRef.value?.signaturePad.isRedrawn()
      )
        return;

      const dataURL = signatureRef.value?.signaturePad.toDataURL(undefined, {
        // 旋转为述评适配预览
        rotation: -90,
      });
      currentDataURL.value = dataURL;
  ...
  render() {
    return (
      ...
      >
        <div class={this.ns.e('modal')} onClick={this.handleModalClick}>
          ...
          {!this.currentDataURL && (
            <span class={this.ns.em('modal', 'caption')}>
              {ibiz.i18n.t('editor.signature.addSignature')}
            </span>
          )}
        </div>
        <div
        ...
        <div
          class={[
            this.ns.b('overlay'),
            this.ns.is('full-screen', this.fullScreen),
        ...
        >
          <div class={this.ns.be('overlay', 'container')}>
            <div class={this.ns.be('overlay', 'header')}>
            ...
            <div class={this.ns.be('overlay', 'footer')}>
