  ...
  implements IDRBarController
{
  ...
  get viewNavPos(): IPanelItemController | undefined {
    return this.view.layoutPanel?.panelItems
      .view_nav_pos as IPanelItemController;
  }
   ...
   */
  handleSelectChange(
    key: string = this.state.selectedItem || this.state.defaultItem,
    isRoutePushed = false,
  ): void {
    ...
    if (drBarItem) {
      ...
      if (this.routeDepth && this.state.drBarItems[0]?.fullPath)
