...
export class AIChatUtil implements IApiAIChatUtil {
  ...
  async getEditorExAIChatParams(
    ...
    // 最大召回数量默认值
    let maxChunksDefaultValue: number =
    ...
    // 召回相似度阈值默认值
    let chunkThresholdDefaultValue: number =
    ...
    chatOptions.fetchKnowledgeBaseList = () => {
      return this.getKnowledgeBaseList(context, params, editorParams);
    };
    // 扩展工具栏
    if (deACMode) {
      const {
  ...
  async getUIActionExAIChatParams(
    ...
    // 知识库
    chatOptions.fetchKnowledgeBaseList = () => {
      return this.getKnowledgeBaseList(context, params);
    };
    // 扩展工具栏
    if (deACMode) {
      const {
