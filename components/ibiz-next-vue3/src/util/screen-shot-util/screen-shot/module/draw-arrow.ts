export class DrawArrow {
  ...
  // 在画布上画出递增变粗的箭头
  private drawArrow(ctx: CanvasRenderingContext2D, color: string): void {
    ctx.fillStyle = color;
    ctx.beginPath();
   ...
   * @private
   */
  private setArrowInfo(edgeLen: number, angle: number): void {
    this.arrowInfo.edgeLen = edgeLen;
    this.arrowInfo.angle = angle;
  }

  // 计算箭头尺寸
  private dynArrowSize(): void {
